...
each containing "Automações" (automations). Tracks the full lifecycle of an automation demand
...
## Tables
1. `frentes` — top-level work fronts (Emana Pay, Ânima, Cresol, DASA...)
2. `clientes` — client organizations
3. `areas` — business areas
4. `stakeholders` — people involved (analyst, GP, SME, leadership, others)
5. `stakeholder_types` — enum-like lookup (analista, gp, cliente, sme, lideranca, outro)
6. `processos` — the core process entity with stage, status, priority, dates
7. `processo_stakeholders` — many-to-many linking people to a process with a role
8. `automacoes` — automations belonging to a process
9. `timeline_events` — append-only history per process and automation
10. `pendencias` — blockers / dependencies / doubts / waiting items per process
11. `checklist_items` — checklist per process and per automation
12. `comentarios` — comments on a process
13. `anexos` — attachments (url + name) on a process
14. `tags` — tag labels
15. `processo_tags` — m2m tags on processes
16. `dependencias` — dependencies between processes or automations

## Security
- RLS enabled on all tables.
- TO anon, authenticated with USING(true) / WITH CHECK(true) — intentionally shared single-user data.
*/

...
/*
# Auth, novos campos em processos, e remoÃ§Ã£o de data_prevista

## VisÃ£o geral
1. Adiciona campos ao `processos`: `sme`, `caminho_anexo`, `volumetria`, `saving`, `arquivado`.
2. Remove `data_prevista` (substituÃ­do por controle de prazo dentro de timeline/pendencias).
3. Tighten RLS: remove polÃ­ticas `anon`; mantÃ©m apenas `authenticated` (login obrigatÃ³rio).

## SeguranÃ§a
- ApÃ³s rodar essa migration, o app EXIGE login. Sem sessÃ£o vÃ¡lida, nenhuma leitura/escrita Ã© permitida.
- Crie o primeiro usuÃ¡rio em: Supabase Dashboard â†’ Authentication â†’ Users â†’ Add user.
- Para desenvolvimento local, vocÃª pode desabilitar confirmaÃ§Ã£o de email: Authentication â†’ Providers â†’ Email â†’ desmarcar "Confirm email".

## Campos removidos
- `processos.data_prevista` â€” use o controle de prazo via `pendencias` ou `timeline_events`.
*/
